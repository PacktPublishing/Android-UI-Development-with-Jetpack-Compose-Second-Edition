                ...
                setContent {
                    MaterialTheme(
                        ...
                        CompositionLocalProvider(
                            LocalWindowSizeClass provides calculateWindowSizeClass(activity = activity)
                        ) {
...
fun SimpleScreen(
    ...
    NumbersList(
        paddingValues = paddingValues, list = list
    )
}
...
@Composable
fun NumbersGrid(
    paddingValues: PaddingValues, list: List<Int>, columns: Int = 2
) {
    LazyVerticalGrid(
...
fun AdaptiveScreen(
    ...
    with(LocalWindowSizeClass.current) {
        if (widthSizeClass == WindowWidthSizeClass.Compact) {
            ...
            NumbersGrid(
                paddingValues = paddingValues, list = list, columns = when (widthSizeClass) {
